...
try:
    ...
    try:
        ...
        print("Error: Could not import loading modules from 'sub' directory.", file=sys.stderr)
        print("Ensure eprload.py is in the parent directory of 'sub', or adjust Python's path.", file=sys.stderr)
        sys.exit(1)
...
def eprload(file_name=None, scaling="", plot_if_possible=True):
    ...
    Args:
        file_name (str or Path, optional): The path to the data file (.dta, .dsc,
            .spc, .par) or a directory. If None or a directory, a file
            browser is shown. Defaults to None (opens browser in cwd).
        scaling (str, optional): A string containing characters to specify
        ...
        plot_if_possible (bool, optional): If True and data is loaded
            ...
            Defaults to True. Set to False to suppress plotting.

    Returns:
        tuple: Depending on success:
            - On success: (x, y, Pars, file_path)
              - x: Abscissa data (numpy array or list of arrays for 2D).
              - y: Ordinate data (numpy array).
              - Pars: Dictionary of parameters from the descriptor/parameter file.
              - file_path: The full path of the loaded file.
            - On failure (e.g., user cancel, file error): (None, None, None, None)

    Raises:
        FileNotFoundError: If the specified file or directory does not exist.
        ValueError: If the file format is unsupported, scaling is invalid,
                    or parameter inconsistencies are found.
    ...
    if not file_extension:
        ...
        for ext in ['.dta', '.DTA', '.dsc', '.DSC', '.spc', '.SPC', '.par', '.PAR']:
             ...
             if potential_file.is_file():
                  found_ext = ext
                  file_path = potential_file # Update file_path to include found extension
                  break
        if found_ext:
             file_extension = found_ext
             warnings.warn(f"No extension given, assuming '{found_ext}' based on existing file.")
        else:
             raise ValueError(f"File '{full_base_name}' lacks a recognized extension (.dta, .dsc, .spc, .par).")
    ...
    # elif ...
         ...
         raise ValueError(f"Unsupported file extension '{file_extension}'. Only Bruker formats (.dta, .dsc, .spc, .par) supported.")
    ...
    if scaling:
        ...
        if invalid_chars:
             raise ValueError(f"Scaling string contains invalid characters: {invalid_chars}. Allowed: '{valid_scaling_chars}'.")
    ...
    try:
        ...
        # Return None tuple on error, matching MATLAB's empty cell behavior on error/cancel
        return None, None, None, None
    except Exception as e:
        # Catch unexpected errors
        warnings.warn(f"An unexpected error occurred loading {file_path}: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
        return None, None, None, None
    ...
    # --- Plotting (Optional) ---
    # Mimic MATLAB's nargout==0 behavior by plotting if requested and successful
...
def _plot_data(
    ...
    if y.ndim == 1:
        ...
        if absc is None or not isinstance(absc, np.ndarray) or absc.shape != y.shape:
            warnings.warn("Abscissa data (x) missing or incompatible shape. Using index for plotting.")
            absc = np.arange(y.size)
            x_label = "Index"
            x_unit = "points"
        else:
            x_label = "Abscissa"
            x_unit = params.get('_Abscissa_Units', 'a.u.') if params else 'a.u.'
            if isinstance(x_unit, list): # Use first unit if list provided for 1D
                x_unit = x_unit[0]
            if x_unit: x_label += f" ({x_unit})"
        ...
        ax.ticklabel_format(style='sci', axis='y', scilimits=(-3,4)) # Use scientific notation if needed
    ...
    # --- 2D Data Plotting ---
        ...
        x_units_list = params.get('_Abscissa_Units') if params else None
        ...
        if isinstance(x, list) and len(x) >= 2:
            ...
            if isinstance(x_axis, np.ndarray) and x_axis.size == nx:
                x_coords = x_axis
                x_unit = params['XUNI'] if isinstance(x_units_list, list) and len(x_units_list) > 0 else 'a.u.'
        ...
        im = ax.pcolormesh(x_coords, y_coords, plot_data, shading='auto', cmap='viridis')
